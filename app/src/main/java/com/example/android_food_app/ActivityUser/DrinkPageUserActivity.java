...
public class DrinkPageUserActivity extends AppCompatActivity {
    ...
    private void getListDrink() {
        ...
        myRef.orderByChild("productType").equalTo("Đồ uống").addChildEventListener(new ChildEventListener() {
            @Override
            public void onChildAdded(@NonNull DataSnapshot snapshot, @Nullable String previousChildName) {
                Product product = snapshot.getValue(Product.class);
                if (product != null) {
                    list.add(product);
                    adapter.setDataDrink(list); // Cập nhật dữ liệu vào adapter
    ...
    private void filter(String text) {
        ...
        adapter.setDataDrink(filteredList);
        adapter.notifyDataSetChanged();
    }
